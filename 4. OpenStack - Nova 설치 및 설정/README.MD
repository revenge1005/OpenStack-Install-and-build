...
#### (2) Nova/Placement 사용자, 서비스, EndPoint 생성
```
...
## 1-2. Controller 노드 - Nova 패키지 설치 및 설정
...
#### (3) 커널 모듈 vhost_net을 활성화하는데 이 vhost_net은 virtio 네트워크 인터페이스를 위해 사용 되며 virtio는 가상화 환경에서 네트워크와 디스크 장치 드라이버를 위해서 사용되는 가상화 표준 이름이다.
```
modprobe vhost_net
```
